...
if (strpos($_SERVER['HTTP_X_SANDSTORM_PERMISSIONS'], "digitalbrowse") !== false) { $xrf_myuclass = $xrf_myuclass . "L"; if ($xrf_myulevel < 2) { $xrf_myulevel = 2; } }
...
if ($xrf_myusername != "Anonymous User" && $xrf_myemail != '')
{
	...
	$xrf_updateuser_query=mysqli_prepare($xrf_db, "UPDATE g_users SET username = ?, lastlogin = now(), ulevel = ?, uclass = ? WHERE sandstormuserid=?") or die(mysqli_error($xrf_db));
	mysqli_stmt_bind_param($xrf_updateuser_query,"siss", $xrf_myusername, $xrf_myulevel, $xrf_myuclass, $xrf_myemail);
